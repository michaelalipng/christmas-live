-- ============================================================================
-- EASY VERSION - Just find and replace 'YOUR_EVENT_ID' with your event UUID
-- ============================================================================
-- STEP 1: Find your event_id by running this query first:
--   SELECT id, name, created_at FROM events ORDER BY created_at DESC LIMIT 5;
--
-- STEP 2: Copy the UUID from the id column
-- 
-- STEP 3: In this file, use Find & Replace (Cmd/Ctrl+F):
--   Find: YOUR_EVENT_ID
--   Replace: (paste your UUID here, like: 123e4567-e89b-12d3-a456-426614174000)
--
...
WITH new_poll AS (
  ...
  VALUES ('YOUR_EVENT_ID', 'In the movie Home Alone where are the McCallisters going on vacation when they leave Kevin behind', 'scheduled', 30, 8)
  RETURNING id
),
options AS (
...
WITH new_poll AS (
  INSERT INTO polls (event_id, question, state, duration_seconds, results_seconds)
  VALUES ('YOUR_EVENT_ID', 'What is the main spice that gives classic gingerbread cookies their flavor', 'scheduled', 30, 8)
  RETURNING id
),
options AS (
  INSERT INTO poll_options (poll_id, label)
...
WITH new_poll AS (
  INSERT INTO polls (event_id, question, state, duration_seconds, results_seconds)
  VALUES ('YOUR_EVENT_ID', 'Which reindeer is famous for having a shiny red nose', 'scheduled', 30, 8)
  RETURNING id
),
options AS (
  INSERT INTO poll_options (poll_id, label)
...
WITH new_poll AS (
  INSERT INTO polls (event_id, question, state, duration_seconds, results_seconds)
  VALUES ('YOUR_EVENT_ID', 'Which country is widely credited with starting the tradition of the decorated Christmas tree', 'scheduled', 30, 8)
  RETURNING id
),
options AS (
  INSERT INTO poll_options (poll_id, label)
...
WITH new_poll AS (
  INSERT INTO polls (event_id, question, state, duration_seconds, results_seconds)
  VALUES ('YOUR_EVENT_ID', 'In the song The Twelve Days of Christmas how many total gifts are given by the end of the song', 'scheduled', 30, 8)
  RETURNING id
),
options AS (
  INSERT INTO poll_options (poll_id, label)
...
WITH new_poll AS (
  INSERT INTO polls (event_id, question, state, duration_seconds, results_seconds)
  VALUES ('YOUR_EVENT_ID', 'What popular Christmas drink is also sometimes called milk punch', 'scheduled', 30, 8)
  RETURNING id
),
options AS (
  INSERT INTO poll_options (poll_id, label)
...
WITH new_poll AS (
  INSERT INTO polls (event_id, question, state, duration_seconds, results_seconds)
  VALUES ('YOUR_EVENT_ID', 'In How the Grinch Stole Christmas what is the name of the Grinch dog', 'scheduled', 30, 8)
  RETURNING id
),
options AS (
  INSERT INTO poll_options (poll_id, label)
...
WITH new_poll AS (
  INSERT INTO polls (event_id, question, state, duration_seconds, results_seconds)
  VALUES ('YOUR_EVENT_ID', 'In the movie A Christmas Story what gift does Ralphie want more than anything', 'scheduled', 30, 8)
  RETURNING id
),
options AS (
  INSERT INTO poll_options (poll_id, label)
...
WITH new_poll AS (
  INSERT INTO polls (event_id, question, state, duration_seconds, results_seconds)
  VALUES ('YOUR_EVENT_ID', 'Which of these is NOT one of Santas reindeer', 'scheduled', 30, 8)
  RETURNING id
),
options AS (
  INSERT INTO poll_options (poll_id, label)
...
WITH new_poll AS (
  INSERT INTO polls (event_id, question, state, duration_seconds, results_seconds)
  VALUES ('YOUR_EVENT_ID', 'What color are the berries of the traditional mistletoe plant', 'scheduled', 30, 8)
  RETURNING id
),
options AS (
  INSERT INTO poll_options (poll_id, label)
...
WITH new_poll AS (
  INSERT INTO polls (event_id, question, state, duration_seconds, results_seconds)
  VALUES ('YOUR_EVENT_ID', 'In which modern day country was Saint Nicholas born', 'scheduled', 30, 8)
  RETURNING id
),
options AS (
  INSERT INTO poll_options (poll_id, label)
...
WITH new_poll AS (
  INSERT INTO polls (event_id, question, state, duration_seconds, results_seconds)
  VALUES ('YOUR_EVENT_ID', 'Which Christmas carol includes the line Fall on your knees', 'scheduled', 30, 8)
  RETURNING id
),
options AS (
  INSERT INTO poll_options (poll_id, label)
...
WITH new_poll AS (
  INSERT INTO polls (event_id, question, state, duration_seconds, results_seconds)
  VALUES ('YOUR_EVENT_ID', 'What is the name for the season of preparation leading up to Christmas in many churches', 'scheduled', 30, 8)
  RETURNING id
),
options AS (
  INSERT INTO poll_options (poll_id, label)
...
WITH new_poll AS (
  INSERT INTO polls (event_id, question, state, duration_seconds, results_seconds)
  VALUES ('YOUR_EVENT_ID', 'In the movie Elf what is the first rule of The Code of Elves', 'scheduled', 30, 8)
  RETURNING id
),
options AS (
  INSERT INTO poll_options (poll_id, label)
...
WITH new_poll AS (
  INSERT INTO polls (event_id, question, state, duration_seconds, results_seconds)
  VALUES ('YOUR_EVENT_ID', 'Which plant is known as the Christmas star due to its bright red leaves', 'scheduled', 30, 8)
  RETURNING id
),
options AS (
  INSERT INTO poll_options (poll_id, label)
...
WITH new_poll AS (
  INSERT INTO polls (event_id, question, state, duration_seconds, results_seconds)
  VALUES ('YOUR_EVENT_ID', 'In A Christmas Carol what is the name of Scrooge cheerful clerk', 'scheduled', 30, 8)
  RETURNING id
),
options AS (
  INSERT INTO poll_options (poll_id, label)
...
WITH new_poll AS (
  INSERT INTO polls (event_id, question, state, duration_seconds, results_seconds)
  VALUES ('YOUR_EVENT_ID', 'What is the name of the famous Christmas ballet featuring Clara and the Sugar Plum Fairy', 'scheduled', 30, 8)
  RETURNING id
),
options AS (
  INSERT INTO poll_options (poll_id, label)
...
WITH new_poll AS (
  INSERT INTO polls (event_id, question, state, duration_seconds, results_seconds)
  VALUES ('YOUR_EVENT_ID', 'In the song Rudolph the Red Nosed Reindeer what do the other reindeer not let Rudolph do', 'scheduled', 30, 8)
  RETURNING id
),
options AS (
  INSERT INTO poll_options (poll_id, label)
...
WITH new_poll AS (
  INSERT INTO polls (event_id, question, state, duration_seconds, results_seconds)
  VALUES ('YOUR_EVENT_ID', 'Under which decoration do people traditionally share a Christmas kiss', 'scheduled', 30, 8)
  RETURNING id
),
options AS (
  INSERT INTO poll_options (poll_id, label)
...
WITH new_poll AS (
  INSERT INTO polls (event_id, question, state, duration_seconds, results_seconds)
  VALUES ('YOUR_EVENT_ID', 'Which of these countries celebrates Christmas during its summer season', 'scheduled', 30, 8)
  RETURNING id
),
options AS (
  INSERT INTO poll_options (poll_id, label)
